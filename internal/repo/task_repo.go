...
func (r *TaskRepo) CreateTask(ctx context.Context, t *model.Task) (int64, error) {
	res, err := r.DB.ExecContext(ctx, "INSERT INTO tasks (id,name,status,delay_time) VALUES (?,?,?,?)", t.ID, t.Name, t.Status, t.DelayTime)
	if err != nil {
		return 0, err
	}
